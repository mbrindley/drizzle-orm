...
class PgDialect {
  ...
  prepareTyping(encoder) {
    if ((0, import_entity.is)(encoder, import_columns.PgColumn)) {
      ...
      if (sqlType === "json" || sqlType === "jsonb") {
        ...
        return "date";
      } else if (sqlType.startsWith("numeric") && /^numeric(\(\d+(,\s*\d+)?\))?$/.test(sqlType)) {
        return "decimal";
      } else if (sqlType.startsWith("time") && /^time(\s*\(\s*(\d+)\s*\))?(\s*(with|without)\s+time\s+zone)?$/.test(sqlType)) {
        return "time";
      } else if (sqlType.startsWith("timestamp") && /^timestamp(\s*\(\s*(\d+)\s*\))?(\s*(with|without)\s+time\s+zone)?$/.test(sqlType)) {
        return "timestamp";
      }
    }
    return "none";
